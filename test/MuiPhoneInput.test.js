...
describe("<PhoneInput /> countries props", () => {
  test('has not "us" country in the dropdown', () => {
    ...
    expect(
      phoneInput.querySelectorAll('li[data-country-code="us"]').length
    ).toBe(0);
    expect(
      phoneInput.querySelectorAll('li[data-country-code="gb"]').length
    ).toBe(1);
  });
  ...
  test('has "us" in the preferred countries section', () => {
    ...
    fireEvent.click(phoneInput.querySelector(".selected-flag"));
    expect(
      phoneInput.querySelector(".country-list").children[0].dataset.countryCode
    ).toBe("us");
  });
});
...
describe("<PhoneInput /> main props", () => {
  test('has "us" as the default/highlighted country', () => {
    ...
    fireEvent.click(phoneInput.querySelector(".selected-flag"));
    expect(
      phoneInput.querySelector(".selected-flag").children[0].classList
    ).toContain("us");
    expect(
      phoneInput.querySelector('li[data-country-code="us"]').classList
    ).toContain("highlight");
  });
...
describe("<PhoneInput /> other props", () => {
  ...
  test("filter european countries with the regions={'europe'} prop", () => {
    ...
    expect(
      phoneInput.querySelectorAll('li[data-country-code="ca"]').length
    ).toBe(0);
    expect(
      phoneInput.querySelectorAll('li[data-country-code="ua"]').length
    ).toBe(1);
    expect(
      phoneInput.querySelectorAll('li[data-country-code="fr"]').length
    ).toBe(1);
  });
  ...
  test('localize countries labels using "localization" prop', () => {
    ...
    expect(
      phoneInput
        .querySelector('li[data-country-code="de"]')
        .querySelector(".country-name").textContent
    ).toBe("Deutschland");
    expect(
      phoneInput
        .querySelector('li[data-country-code="es"]')
        .querySelector(".country-name").textContent
    ).toBe("España");
  });
  ...
  test("search correct country via search field", () => {
    ...
    expect(
      phoneInput
        ...
        .children[1].querySelector(".country-name").textContent
    ).toBe("United Kingdom");
  });
...
describe("correct value update", () => {
  test("should rerender without crashing", () => {
    ...
    expect(
      phoneInput.querySelector(".selected-flag").children[0].className
    ).toBe("flag 0");
  });
